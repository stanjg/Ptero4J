        ...
        try {
            ...
            if (json.has("attributes")) {
                ...
                if (json.has("memory") && json.has("disk") && json.has("cpu"))
                    if (json.getJSONObject("cpu").getFloat("limit") != 0)
                        return new ServerUsage(Math.round((json.getJSONObject("cpu").getFloat("current") / json.getJSONObject("cpu").getFloat("limit")) * 100), json.getJSONObject("memory").getInt("current"), json.getJSONObject("disk").getInt("current"));
                    else return null;
                else return null;
            } else {
